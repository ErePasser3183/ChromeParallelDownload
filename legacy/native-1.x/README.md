# Chrome Parallel Download 1.x（历史原生版）

本目录归档 Windows 原生版 1.1.4 的源码、测试与安装脚本。当前主工程请见 [仓库首页](../../README.md)。

## 目录迁移说明

以下所有命令都在本目录运行。若此前安装过原生版，原有注册路径已随工程整理改变；需要重新使用时，先在本目录运行 `install.cmd` 重新注册，再加载本目录内的 `extension`。本机生成配置仍由 Git 忽略，不随源码发布。

本页的 Python / CMD 要求只属于历史原生版。
