...
async function update() {
  ...
  try {
    ...
    $('engine').textContent=data.engine.error?'引擎未连接':data.engine.connecting?'正在连接引擎…':'aria2 '+data.engine.version+' · 已连接';
...
chrome.storage.onChanged.addListener(changes => {if(changes.tasks) void update();});
