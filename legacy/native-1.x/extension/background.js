...
async function persist() {
  ...
  const preparing = Object.values(tasks).some(t => t.status === 'preparing');
  await chrome.action.setBadgeText({text: preparing ? '…' : String(Object.values(tasks).filter(t => t.status === 'active').length || '')});
}
...
chrome.webRequest.onBeforeSendHeaders.addListener(details => {
  ...
  requests.set(details.url, {method: details.method, privateRequest, headers, time: Date.now(),
    requestId: details.requestId, tabId: details.tabId});
