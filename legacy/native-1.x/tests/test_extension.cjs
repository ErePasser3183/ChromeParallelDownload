...
function harness(seed={}) {
  ...
  const listeners={}; let failPing=false, failStart=false, holdPing=false;
  ...
  const chrome={
    ...
    runtime:{id:'test',onMessage:event('message'),connectNative:()=>({
      ...
      postMessage:m=>{calls.push(m.action);if(m.action==='ping'&&holdPing)return;queueMicrotask(()=>{
  ...
  return {store,downloads,nativeJobs,calls,listeners,context,setFailPing:()=>failPing=true,setFailStart:()=>failStart=true,setHoldPing:()=>holdPing=true};
}
...
(async()=>{
  if (process.argv.includes('--feedback')) {
    const h=harness();await run(h,'ready');await new Promise(setImmediate);h.setHoldPing();item(h);
    void run(h,'intercept(testItem)');await new Promise(setImmediate);
    assert.equal(Object.values(h.store.tasks)[0]?.status,'preparing','Task must be visible before a delayed native handshake finishes');
    const started=performance.now();
    const result=await Promise.race([new Promise(resolve=>h.listeners.message({action:'view'},{id:'test'},resolve)),new Promise(resolve=>setTimeout(()=>resolve(null),200))]);
    assert(result?.ok,'Popup must render cached tasks without waiting for the engine');
    assert(performance.now()-started<200);
    console.log('Immediate accepted task + nonblocking popup with unresponsive engine: PASS');process.exit(0);
  }
